...
beq $t1,$s2,done # si se recorrio todo el array
...
beq $t4,$zero,max # si no es verdad vuelve al inicio
addi $t0,$t3,0 # si es verdad A[i] es el nuevo maximo
j max # vuelve al inicio

sort: beq $a0,$a1,done # single-element list is sorted
jal max # call the max procedure
lw $t5,0($a1) # load last element into $t0
sw $t5,0($v0) # copy the last element to max loc
